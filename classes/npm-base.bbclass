...
oe_runnpx() {
    ...
    bbnote ${NPX} --registry=${NPM_REGISTRY} ${ARCH_FLAGS} ${NPM_FLAGS} "$@"
    ...
    ${NPX} cache $NPM_CACHE_CMD || die "oe_runnpx failed (cache $NPM_CACHE_CMD)"

    LD="${NPM_LD}" ${NPX} --registry=${NPM_REGISTRY} ${ARCH_FLAGS} ${NPM_FLAGS} "$@" || die "oe_runnpx failed (install)"
}
