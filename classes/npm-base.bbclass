...
oe_runnpx() {
    ...
    bbnote ${NPX} --userconfig=${NPMRC} ${ARCH_FLAGS} ${NPM_FLAGS} "$@"
    ...
    LD="${NPM_LD}" ${NPX} --userconfig=${NPMRC} ${ARCH_FLAGS} ${NPM_FLAGS} "$@" || die "oe_runnpx failed (install)"
}
